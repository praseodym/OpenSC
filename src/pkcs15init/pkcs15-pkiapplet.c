/*
 * PKIapplet specific operation for PKCS15 initialization
 *
 * Copyright (C) blabla
 *
...
#define PKIAPPLET_PIN_ID_MIN 1
#define PKIAPPLET_PIN_ID_MAX 15

/*
 * Allocate a file
 */
static int
pkiapplet_new_file(struct sc_profile *profile, sc_card_t *card,
...
static int
pkiapplet_update_private_key(struct sc_profile *profile, sc_card_t *card,
 ...
 */
static int
pkiapplet_create_dir(struct sc_profile *profile, sc_pkcs15_card_t *p15card,
		struct sc_file *df)
{
	SC_FUNC_CALLED(profile, SC_LOG_DEBUG_VERBOSE);
 ...
 */
static int
pkiapplet_select_pin_reference(struct sc_profile *profile, sc_pkcs15_card_t *p15card,
		struct sc_pkcs15_auth_info *auth_info)
{
	if (auth_info->auth_type != SC_PKCS15_PIN_AUTH_TYPE_PIN)
		return SC_ERROR_OBJECT_NOT_VALID;

	if (auth_info->attrs.pin.reference < PKIAPPLET_PIN_ID_MIN)
		auth_info->attrs.pin.reference = PKIAPPLET_PIN_ID_MIN;
 ...
 */
static int
pkiapplet_create_pin(struct sc_profile *profile, sc_pkcs15_card_t *p15card, struct sc_file *df,
		...
		const u8 *puk, size_t puk_len)
{
	...
	struct sc_cardctl_pkiapplet_ac_info ac_info;
	int r;
	...
	//r = sc_card_ctl(p15card->card, SC_CARDCTL_PKIAPPLET_CREATE_AC, &ac_info);
    //    SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, r, "PKIapplet create AC failed");
 ...
 */
static int
pkiapplet_create_key(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
		struct sc_pkcs15_object *object)
{
	...
	if (object->type != SC_PKCS15_TYPE_PRKEY_RSA)
        	SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, SC_ERROR_NOT_SUPPORTED, "PKIapplet supports only 1024-bit RSA keys.");

	if (key_info->modulus_length != 1024)
        	SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, SC_ERROR_NOT_SUPPORTED, "PKIapplet supports only 1024-bit RSA keys.");

        sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "create private key ID:%s\n",  sc_pkcs15_print_id(&key_info->id));
	r = pkiapplet_new_file(profile, p15card->card, SC_PKCS15_TYPE_PRKEY_RSA, key_info->key_reference, &file);
 ...
 */
static int
pkiapplet_store_key(struct sc_profile *profile, struct sc_pkcs15_card *p15card,
		struct sc_pkcs15_object *object,
		struct sc_pkcs15_prkey *key)
{
	...
	r = pkiapplet_update_private_key(profile, p15card->card, rsa);

	SC_FUNC_RETURN(ctx, SC_LOG_DEBUG_NORMAL, r);
}

static struct sc_pkcs15init_operations sc_pkcs15init_pkiapplet_operations = {
	NULL,				/* erase_card */
	NULL,				/* init_card  */
	pkiapplet_create_dir,
	NULL,				/* create_domain */
	pkiapplet_select_pin_reference,
	pkiapplet_create_pin,
	NULL,				/* select_key_reference */
	pkiapplet_create_key,
	pkiapplet_store_key,
	NULL,				/* generate_key */
...
struct sc_pkcs15init_operations *sc_pkcs15init_get_pkiapplet_ops(void)
{
	printf("\nPKCS15INIT GETOPS PKIAPPLET\n\n");
	
	return &sc_pkcs15init_pkiapplet_operations;
}
